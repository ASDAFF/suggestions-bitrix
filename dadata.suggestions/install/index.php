...
class dadata_suggestions extends CModule
{
        ...
        if (is_array($this->NEED_MODULES) && !empty($this->NEED_MODULES)) {
            foreach ($this->NEED_MODULES as $module) {
                if (!IsModuleInstalled($module)) {
                    $this->ShowForm('ERROR', GetMessage('ASD_DADATA_NEED_MODULES', array('#MODULE#' => $module)));
                }
            }
        }
        if (strlen($this->NEED_MAIN_VERSION) <= 0 || version_compare(SM_VERSION, $this->NEED_MAIN_VERSION) >= 0) {
            if ($this->InstallDB()) {
                ...
                if ($ex = $APPLICATION->GetException()) {
                    ...
                    $strError = GetMessage('ASD_DADATA_UNKNOWN_ERR_INSTALL');
                }
                $this->ShowForm('ERROR', GetMessage('ASD_DADATA_NOT_INSTALL', array('#ERR#' => $strError)));
            }
        } else {
            $this->ShowForm('ERROR', GetMessage('ASD_DADATA_NEED_RIGHT_VER', array('#NEED#' => $this->NEED_MAIN_VERSION)));
        }
    }
    ...
    public function DoUninstall()
    {
        ...
        if ($this->UnInstallDB()) {
            ...
            if ($ex = $APPLICATION->GetException()) {
                ...
                $strError = GetMessage('ASD_DADATA_UNKNOWN_ERR_UNINSTALL');
            }
            $this->ShowForm('ERROR', GetMessage('ASD_DADATA_NOT_UNINSTALL', array('#ERR#' => $strError)));
        }
    }
    ...
    private function ShowForm($type, $message, $mode = '')
    {
        ...
        $GLOBALS['APPLICATION']->SetTitle(GetMessage('ASD_DADATA_MODULE_NAME'));
        ...
        <form action="<?= $GLOBALS['APPLICATION']->GetCurPage() ?>" method="get">
            <? if ($mode == 'install'): ?>
                <?= GetMessage('ASD_DADATA_MODULE_SETTINGS', array('#LANG#' => LANGUAGE_ID)); ?>
            <? endif; ?>
            <p>
